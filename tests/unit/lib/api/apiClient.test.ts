...
const BASE_URL = 'https://nextcloud.example.com';
const API_PATH = '/index.php/apps/news/api/v1-3';
...
describe('ApiClient', () => {
  ...
  describe('version endpoint group', () => {
    it('should fetch version without authentication', async () => {
      const client = getApiClient();
      const result = await client.version.get('https://nextcloud.example.com');
    ...
    it('should not require session for version endpoint', async () => {
      ...
      const client = getApiClient();
      await expect(client.version.get('https://nextcloud.example.com')).resolves.not.toThrow();
    });
  });
  ...
  describe('auth utilities', () => {
    it('should validate correct credentials', async () => {
      const client = getApiClient();
      const result = await client.auth.validateCredentials(BASE_URL, TEST_CREDENTIALS);
    ...
    it('should reject invalid credentials', async () => {
      ...
      const client = getApiClient();
      const result = await client.auth.validateCredentials(BASE_URL, 'invalid');
